#!/bin/sh

# Script to configure a crossed-native binutils for MinGW.

# IMPORTANT:
# ----------
# a. You should have built and installed cross-binutils and GCC
#    cross-compiler for MinGW in the folder $XGCC_DIR.
#
# b. You should have installed mingw-runtime and w32api binaries in the
#    folder $SYSROOT/mingw.
#

# You MUST update these variables to reflect your setup.
#
BU_SRC_DIR=/extra/src/binutils-2.16.1
XGCC_DIR=/extra/xgcc
PREFIX=/extra/wgcc


# You do not have to change anything below this, but it does not
# hurt to review it anyway.

SYSROOT=$PREFIX/sys-root
TARGET=i686-pc-mingw32
BUILD=`$BU_SRC_DIR/config.guess`
HOST=$TARGET

CC=$XGCC_DIR/bin/$TARGET-gcc
export CC

CXX=$XGCC_DIR/bin/$TARGET-g++
export CXX

AS=$XGCC_DIR/bin/$TARGET-as
export AS

LD=$XGCC_DIR/bin/$TARGET-ld
export LD

AR=$XGCC_DIR/bin/$TARGET-ar
export AR

PATH=$XGCC_DIR/bin:$PATH
export PATH

$BU_SRC_DIR/configure --prefix="$PREFIX" --with-sysroot="$SYSROOT" \
    --build=$BUILD --host=$HOST --target=$TARGET \
    --disable-nls --disable-shared --disable-debug --enable-threads=win32
